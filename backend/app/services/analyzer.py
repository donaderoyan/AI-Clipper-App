...
def extract_highlights_from_text(transcript: str, prompt_context: str, segments: List[dict], target_duration: Optional[int] = None, output_count: int = 1) -> List[Dict[str, float]]:
    if target_duration:
    ...
    for i in range(output_count):
        ...
        example_json += f'  {{"start": {start_time}, "end": {end_time}, "label": "Hook utama {i+1}"}}'
        if i < output_count - 1:
            example_json += ",\n"
    example_json += "\n]"
